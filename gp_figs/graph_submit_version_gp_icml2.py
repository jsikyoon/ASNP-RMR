...
dirs_a = [
    ...
    '01-21,21:09:18.184695', #SNPK(h=128,K=9)
    '01-21,10:46:51.922937', #SNPK(h=128,K=25)
    '01-20,22:32:44.665775', #SNPK(h=128,K=100)
    '11-11,18:59:57.248192', #SNPK(h=128,K=inf)
    '01-20,22:31:33.956285', #ASNP(h=128,K=9)
    '09-06,17:57:56.149375', #ASNP(h=128,K=25)
    '01-20,22:30:44.735277', #ASNP(h=128,K=100)
    ]
...
dirs_b = [
    ...
    '01-20,22:30:47.411256', #SNPK(h=128,K=9)
    '01-20,22:30:44.671857', #SNPK(h=128,K=25)
    '01-20,18:34:46.370648', #SNPK(h=128,K=100)
    '11-11,18:17:54.618255', #SNPK(h=128,K=inf)
    '01-20,11:35:00.225511', #ASNP(h=128,K=9)
    '09-06,17:59:54.232008', #ASNP(h=128,K=25)
    '01-20,10:27:38.700624', #ASNP(h=128,K=100)
     ]
...
for d_idx, dir_name in enumerate(dirs_a):
    ...
    if time_printing:
        ...
        for t_idx, (t, nll) in enumerate(zip(times,tnll)):
            if t_idx % 50 == 0:
                outstr+='('+str(t/1000000)+','+str(nll[0])+')'
...
for d_idx, dir_name in enumerate(dirs_c):
    ...
    if k_printing:
        times = np.array([e.wall_time for e in summ[0].Scalars(tag)])
        times = times - times[0]
        tnll_list.append(out['TargetNLL/Nll'])
...
min_tnll = 100000000000000
for i in range(len(dirs_c)):
    if min_tnll > len(tnll_list[i]):
        min_tnll = len(tnll_list[i])

print(min_tnll)
for i in range(len(dirs_c)):
    print('task c',labels[i])
    print(np.mean(tnll_list[i][(min_tnll-100):min_tnll]))
