...
img_name = 'mnist_case'+str(case)+'_'
...
if case==1:
    ...
    # case 3
    dirs = [
        'logs_c/07-25,10:24:11.220550',  # NP
        'logs_c/07-25,10:24:15.632419',  # ANP
        'logs_c/07-25,10:24:13.559183',  # SNP
        'logs_c/07-25,10:24:09.806928',  # ASNP
       ]
else:
...
for t_idx, t in enumerate(T):
    for i in range(len(labels)):
        ...
        for j in range(len(tar_x)):
            ...
            y_loc = int(tar_x[j][1])
            if i == 0:
                tar_canvas[x_loc][y_loc] = tar_y[j][0]
            pred_canvas[x_loc][y_loc] = np.clip(pre_y[j][0],0,1)
            std_canvas[x_loc][y_loc] = np.clip(std_y[j][0],0,1)
        ...
        if i == 0:
            for j in range(len(con_x)):
                ...
                cont_canvas[x_loc][y_loc][0] = con_y[j][0]
